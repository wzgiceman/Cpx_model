 ...
 * Company :cpx
 */
class VideoAdapter(context: Context) : RecyclerArrayAdapter<Video.EntriesBean>(context) {
     ...
     * 操作的逻辑必须踢出到外层处理
     */
    internal class WaterHolder(parent: ViewGroup, private val random: Random) : BaseHolder<Video.EntriesBean>(parent, R.layout
            .recycler_item_vedio_holder_recycler) {
        private var ivWall: ImageView = `$`(R.id.iv_wall)

        @SuppressLint("ResourceType")
        override fun setData(data: Video.EntriesBean) {
