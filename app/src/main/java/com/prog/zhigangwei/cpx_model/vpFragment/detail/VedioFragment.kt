 ...
 *Company :cpx
 */
class VedioFragment : BaseLazyFragment(), HttpOnNextListener {
    private val adapter by lazy { VedioAdapter(context) }
    ...
    override fun onNext(result: String, method: String) {
        ...
        adapter.removeAll()
        adapter.addAll(JSONObject.parseObject(result, Vedio::class.java).entries)
    }
