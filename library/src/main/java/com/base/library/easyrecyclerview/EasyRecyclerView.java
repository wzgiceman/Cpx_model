...
public class EasyRecyclerView extends FrameLayout {
    ...
    /**
     * 设置下拉刷新、EmptyView点击、ErrorView点击listener
     * 一般来说这三种情况的触发事件都是一样的，所以简单的做个封装
     * @param listener 监听器
     */
    public void setRefreshAndRetryListener(android.support.v4.widget.SwipeRefreshLayout.OnRefreshListener listener) {
        mPtrLayout.setEnabled(true);
        mPtrLayout.setOnRefreshListener(listener);
        this.mRefreshListener = listener;
        if (mEmptyView != null) {
            mEmptyView.setOnClickListener(v -> listener.onRefresh());
        }
        if (mErrorView != null) {
            mErrorView.setOnClickListener(v -> listener.onRefresh());
        }
    }
